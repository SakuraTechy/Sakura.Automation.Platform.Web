  ...
  <div>
    ...
    <advance-table style="margin-top: -20px;" :scroll="{ x: 1500, y: 217 }" :columns="columns" :data-source="list"
      ...
      <div class="table-operations" slot="button">
        <a-button type="primary" @click="handleAdd()" v-hasPermi="['project:environmentConfig:domainConfig:add']">
          <a-icon type="plus-square" />新建域名
        </a-button>
        <a-space :size="8" style="margin-right: 8px">
          <a-button type="primary" @click="handleImport" v-hasPermi="['project:environmentConfig:domainConfig:export']">
            <a-icon type="upload" />批量导入
          </a-button>
          <a-button type="danger" :disabled="multiple" @click="handleExport"
            v-hasPermi="['project:environmentConfig:domainConfig:export']">
            <a-icon type="download" />批量导出
          </a-button>
          <a-button type="danger" :disabled="multiple" @click="handleDelete"
            v-hasPermi="['project:environmentConfig:domainConfig:remove']">
      ...
      <span slot="operation" slot-scope="{ record }">
        <a @click="handleView(record)" v-hasPermi="['project:environmentConfig:domainConfig:query']"> 查看 </a>
        <a-divider type="vertical" />
        <a @click="handleUpdate(record)" v-hasPermi="['project:environmentConfig:domainConfig:edit']"> 修改 </a>
        <a-divider type="vertical" />
        <a @click="handleCopy(record)" v-hasPermi="['project:environmentConfig:domainConfig:edit']"> 复制 </a>
        <a-divider type="vertical" />
        <a @click="handleDelete(record)" v-show="record.id !== '1'"
          v-hasPermi="['project:environmentConfig:domainConfig:remove']">删除</a>
        <a-divider type="vertical" />
        <a @click="handleExport(record)" v-hasPermi="['project:environmentConfig:domainConfig:export']"> 导出 </a>
      </span>
    </advance-table>
  ...
  methods: {
    getList() {
      ...
      projectApis.getEnvironmentInfo(this.environment_Id).then((response) => {
        if(response.data.domainConfig.length > 0){
          this.list = JSON.parse(response.data.domainConfig)
        }else{
          this.list = []
        }
