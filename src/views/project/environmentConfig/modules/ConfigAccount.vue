  ...
  <div>
    ...
    <advance-table style="margin-top: -20px;" :scroll="{ x: 1500, y: 217 }" :columns="columns" :data-source="list"
      ...
      <div class="table-operations" slot="button">
        <a-button type="primary" @click="handleAdd()" v-hasPermi="['project:environmentConfig:accountConfig:add']">
          <a-icon type="plus-square" />新建帐号
        </a-button>
        <a-space :size="8" style="margin-right: 8px">
          <a-button type="primary" @click="handleImport" v-hasPermi="['project:environmentConfig:accountConfig:export']">
            <a-icon type="upload" />批量导入
          </a-button>
          <a-button type="danger" :disabled="multiple" @click="handleExport"
            v-hasPermi="['project:environmentConfig:accountConfig:export']">
            <a-icon type="download" />批量导出
          </a-button>
          <a-button type="danger" :disabled="multiple" @click="handleDelete"
            v-hasPermi="['project:environmentConfig:accountConfig:remove']">
            <a-icon type="delete" />批量删除
          </a-button>
        </a-space>
      </div>
      <span slot="type" slot-scope="{ record }">
        <a v-for="(item, value) in accountType" v-if="item.value===record.type" :key="value" :value="item.value">
          {{ item.label }}
        </a>
      </span>
      ...
      <span slot="operation" slot-scope="{ record }">
        <a @click="handleView(record)" v-hasPermi="['project:environmentConfig:accountConfig:query']"> 查看 </a>
        <a-divider type="vertical" />
        <a @click="handleUpdate(record)" v-hasPermi="['project:environmentConfig:accountConfig:edit']"> 修改 </a>
        <a-divider type="vertical" />
        <a @click="handleCopy(record)" v-hasPermi="['project:environmentConfig:accountConfig:edit']"> 复制 </a>
        <a-divider type="vertical" />
        <a @click="handleDelete(record)" v-show="record.id !== '1'"
          v-hasPermi="['project:environmentConfig:accountConfig:remove']">删除</a>
        <a-divider type="vertical" />
        <a @click="handleExport(record)" v-hasPermi="['project:environmentConfig:accountConfig:export']"> 导出 </a>
      </span>
    </advance-table>
...
import { accountType, accountQueryData, accountColumns } from './EnvConfig'
  ...
  methods: {
    getList() {
      ...
      projectApis.getEnvironmentInfo(this.environment_Id).then((response) => {
        if (response.data.accountConfig.length > 0) {
          this.list = JSON.parse(response.data.accountConfig)
        } else {
          this.list = []
        }
        this.list1 = this.list
        this.loading = false
        // this.list = JSON.parse(response.data.accountConfig) > 0 ? this.list : []
    ...
    onSelectChange(selectedRowKeys, selectedRows) {
      ...
      this.names = this.selectedRows.map(item => item.userName)
    ...
    onchange(value, record) {
      ...
      projectApis.editAccount(params).then(response => {
        this.getList()
