  ...
  <div>
    ...
    <advance-table style="margin-top: -20px;" :scroll="{ x: 1500, y: 217 }" :columns="columns" :data-source="list"
      ...
      <div class="table-operations" slot="button">
        ...
        <a-space :size="8" style="margin-right: 8px">
          ...
          <a-button type="danger" :disabled="multiple" @click="handleExport"
            v-hasPermi="['project:environmentConfig:serverConfig:export']">
            <a-icon type="download" />批量导出
          </a-button>
          <a-button type="danger" :disabled="multiple" @click="handleDelete"
            v-hasPermi="['project:environmentConfig:serverConfig:remove']">
            <a-icon type="delete" />批量删除
          </a-button>
        </a-space>
      </div>
      <span slot="type" slot-scope="{ record }">
        <a v-for="(item, value) in serverType" v-if="item.type===record.type" :key="value" :value="item.type">
          {{ item.label }}
        </a>
      </span>
      ...
      <span slot="operation" slot-scope="{ record }">
        <a @click="handleView(record)" v-hasPermi="['project:environmentConfig:serverConfig:query']"> 查看 </a>
        <a-divider type="vertical" />
        <a @click="handleUpdate(record)" v-hasPermi="['project:environmentConfig:serverConfig:edit']"> 修改 </a>
        <a-divider type="vertical" />
        <a @click="handleCopy(record)" v-hasPermi="['project:environmentConfig:serverConfig:edit']"> 复制 </a>
        <a-divider type="vertical" />
...
export default {
  ...
  methods: {
    getList() {
      ...
      projectApis.getEnvironmentInfo(this.environment_Id).then((response) => {
        if (response.data.serverConfig.length > 0) {
          this.list = JSON.parse(response.data.serverConfig)
        } else {
          this.list = []
        }
    ...
    onSelectChange(selectedRowKeys, selectedRows) {
      ...
      this.names = this.selectedRows.map(item => item.host)
