  ...
  <div>
    ...
    <advance-table style="margin-top: -20px;" :scroll="{ x: 1500, y: 217 }" :columns="columns" :data-source="list"
      ...
      <div class="table-operations" slot="button">
        <a-button type="primary" @click="handleAdd()" v-hasPermi="['project:environmentConfig:versionConfig:add']">
          <a-icon type="plus-square" />新建版本
        </a-button>
        <a-space :size="8" style="margin-right: 8px">
          <a-button type="primary" @click="handleImport" v-hasPermi="['project:environmentConfig:versionConfig:export']">
            <a-icon type="upload" />批量导入
          </a-button>
          <a-button type="danger" :disabled="multiple" @click="handleExport"
            v-hasPermi="['project:environmentConfig:versionConfig:export']">
            <a-icon type="download" />批量导出
          </a-button>
          <a-button type="danger" :disabled="multiple" @click="handleDelete"
            v-hasPermi="['project:environmentConfig:versionConfig:remove']">
      ...
      <!-- <span slot="status" slot-scope="{ record }">
                              <a-switch :default-checked="Boolean(record.status)" @change="(value) => onchange(value, record)" />
                            </span> -->
      <!-- <span slot="status" slot-scope="{ record }">
                              <a-switch :checked="record.status === 1 ? true : false" @change="(value) => onchange(value, record)" />
                            </span> -->
      <span slot="status" slot-scope="{ record }">
        <a-switch :checked="Boolean(record.status)" @change="(value) => onchange(value, record)" />
      </span>
      <!-- <span slot="status" slot-scope="{ record }">
                              <a-switch :checked="record.status === '0' ? true : false" @change="onChangeStatus(record)"
      ...
      <span slot="operation" slot-scope="{ record }">
        <a @click="handleView(record)" v-hasPermi="['project:environmentConfig:versionConfig:query']"> 查看 </a>
        <a-divider type="vertical" />
        <a @click="handleUpdate(record)" v-hasPermi="['project:environmentConfig:versionConfig:edit']"> 修改 </a>
        <a-divider type="vertical" />
        <a @click="handleCopy(record)" v-hasPermi="['project:environmentConfig:versionConfig:edit']"> 复制 </a>
        <a-divider type="vertical" />
        <a @click="handleDelete(record)" v-show="record.id !== '1'"
          v-hasPermi="['project:environmentConfig:versionConfig:remove']">删除</a>
        <a-divider type="vertical" />
        <a @click="handleExport(record)" v-hasPermi="['project:environmentConfig:versionConfig:export']"> 导出 </a>
      </span>
    </advance-table>
...
import { versionQueryData, versionColumns } from './EnvConfig'
export default {
  name: "ConfigVersion",
  props: {
    environment_Id: {
  ...
  methods: {
    handleQuery(value) {
      if (value === undefined) {
        ...
        if (this.versionList.length > 0) {
          this.list = this.getList1(this.versionList, value)
        }
        this.loading = false
        // this.versionList.length > 0 ? this.list = this.getList1(this.versionList, value) : this.loading = false
      }
    },
    resetQuery() {
      this.queryParam = {
    ...
    getList() {
      if(this.environment_Id===''){
        return this.$message.warning('请先保存环境！')
      }
      this.loading = true
    ...
    handleExport() { },
    // 表格内-状态切换
    onchange(value, { id, name, description }) {
      let params = {
