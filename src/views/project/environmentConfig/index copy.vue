      ...
      <a-tabs v-model="activeKey" class="tab-title" default-active-key="1" type="editable-card" hide-add @edit="onEdit">
        <a-tab-pane key="1" tab="环境列表" :closable="false">
          ...
          <advance-table :scroll="{ x: 1500, y: 500 }" :columns="columns" :data-source="list" :loading="loading"
            ...
            tableKey="system-environment-index-table" :format-conditions="true" :pagination="{
            ...
            <div class="table-operations" slot="button">
              <div>
                <a-button type="primary" size="small" @click="handleAdd()" style="margin-right: 16px; "
                  v-hasPermi="['projectSetting:environment:add']">
        ...
        <a-tab-pane v-for="(item, index) in panes" :key="(index + 2).toString()" :tab="item.title" :closable="true">
          <EnvConfig :record="item.record"></EnvConfig>
        </a-tab-pane>
      </a-tabs>
    </a-card>
  </div>
...
<script>
import { getEnvironmentList, deleteEnvironment, getAllProject, changeEnvStatus } from '@/api/project'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      var that = this
      const ids = [row.id]
      const environmentNames = row.name || this.environmentNames
      this.$Modal.confirm({
        title: '确认删除所选中数据?',
    ...
    // 表格内-状态切换
    onchange(value, { status, projectId, id }) {
      let params = {
        status: Number(value),
        projectId,
        id
      }
      changeEnvStatus(params).then(() => {
        this.getList()
