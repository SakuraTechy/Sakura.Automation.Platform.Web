  ...
  <div>
    ...
    <advance-table style="margin-top: -20px;" :scroll="{ x: 1500, y: 176 }" :columns="columns" :data-source="list"
      ...
      <div class="table-operations" slot="button">
        <a-button type="primary" :disabled="multiple" @click="handleAdd()" v-hasPermi="['project:automationConfig:jenkinsConfig:add']">
          <a-icon type="plus-square" />新建Jenkins配置
        </a-button>
        <a-space :size="8" style="margin-right: 8px">
          <a-button type="primary" :disabled="multiple" @click="handleImport" v-hasPermi="['project:automationConfig:jenkinsConfig:export']">
            <a-icon type="upload" />批量导入
          </a-button>
          <a-button type="danger" :disabled="multiple1" @click="handleExport"
            v-hasPermi="['project:automationConfig:jenkinsConfig:export']">
            <a-icon type="download" />批量导出
          </a-button>
          <a-button type="danger" :disabled="multiple1" @click="handleDelete"
            v-hasPermi="['project:automationConfig:jenkinsConfig:remove']">
      ...
      <span slot="operation" slot-scope="{ record }">
        <a @click="handleView(record)" v-hasPermi="['project:automationConfig:jenkinsConfig:query']"> 查看 </a>
        <a-divider type="vertical" />
        <a :disabled="multiple" @click="handleUpdate(record)" v-hasPermi="['project:automationConfig:jenkinsConfig:edit']"> 修改 </a>
        <a-divider type="vertical" />
        <a :disabled="multiple" @click="handleCopy(record)" v-hasPermi="['project:automationConfig:jenkinsConfig:edit']"> 复制 </a>
        <a-divider type="vertical" />
        <a :disabled="multiple" @click="handleDelete(record)" v-show="record.id !== '1'"
          v-hasPermi="['project:automationConfig:jenkinsConfig:remove']">删除</a>
        <a-divider type="vertical" />
        <a :disabled="multiple" @click="handleExport(record)" v-hasPermi="['project:automationConfig:jenkinsConfig:export']"> 导出 </a>
      </span>
    </advance-table>
...
import { jenkinsQueryData, jenkinsColumns } from './Config'
  ...
  methods: {
    getList() {
      ...
      projectApis.getAutomationInfo(this.automation_Id).then((response) => {
        if (response.data.jenkinsConfig !== null && response.data.jenkinsConfig.length > 0) {
          this.list = JSON.parse(response.data.jenkinsConfig)
        } else {
          this.list = []
        }
    ...
    onSelectChange(selectedRowKeys, selectedRows) {
      ...
      this.names = this.selectedRows.map(item => item.ip)
    ...
    handleAdd() {
      ...
      this.$nextTick(() => (
        this.$refs.ConfigDataAddOrEdit.handleAdd(2)
      ))
    },
    handleView(record) {
      this.showAddModal = true
      this.$nextTick(() => (
        this.$refs.ConfigDataAddOrEdit.handleView(record,2)
      ))
    },
    handleUpdate(record) {
      this.showAddModal = true
      this.$nextTick(() => (
        this.$refs.ConfigDataAddOrEdit.handleUpdate(record, 2)
      ))
    },
    handleCopy(record) {
      this.showAddModal = true
      this.$nextTick(() => (
        this.$refs.ConfigDataAddOrEdit.handleCopy(record, 2)
      ))
    },
    handleDelete(row) {
      this.showAddModal = true
      if (row.id !==undefined || this.selectedRowKeys.length > 0 ) {
        this.$nextTick(() => (
          this.$refs.ConfigDataAddOrEdit.handleDelete(row, this.ids, this.names, 2)
        ))
      } else {
