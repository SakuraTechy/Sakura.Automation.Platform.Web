<template>
  <div>
    <SearchControl :queryData="queryData" @handleQuery="handleQuery" @resetQuery="resetQuery" :multiple="multiple" />
    ...
    <advance-table style="margin-top: -20px;" :scroll="{ x: 1500, y: 176 }" :columns="columns" :data-source="list"
      ...
      <div class="table-operations" slot="button">
        <a-button type="primary" :disabled="multiple" @click="handleAdd()"
          v-hasPermi="['project:automationConfig:projectConfig:add']">
          <a-icon type="plus-square" />新建项目配置
        </a-button>
      ...
      <span slot="operation" slot-scope="{ record }">
        ...
        <a :disabled="multiple" @click="handleUpdate(record)"
          v-hasPermi="['project:automationConfig:projectConfig:edit']"> 修改 </a>
        <a-divider type="vertical" />
        <a :disabled="multiple" @click="handleCopy(record)" v-hasPermi="['project:automationConfig:projectConfig:edit']">
          复制 </a>
        <a-divider type="vertical" />
        ...
        <a :disabled="multiple" @click="handleExport(record)"
          v-hasPermi="['project:automationConfig:projectConfig:export']"> 导出 </a>
      </span>
    </advance-table>
  ...
  methods: {
    getList() {
      ...
      projectApis.getAutomationInfo(this.automation_Id).then((response) => {
        if (response.data.projectConfig !== null && response.data.projectConfig.length > 0) {
