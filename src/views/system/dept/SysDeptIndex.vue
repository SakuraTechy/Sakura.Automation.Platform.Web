          ...
          <a-row :gutter="48">
            <a-col :md="6" :sm="24">
              <a-form-item label="部门名称">
                <a-input v-model="queryParam.deptName" placeholder="请输入部门名称" allow-clear @keyup.enter.native="handleQuery"/>
              </a-form-item>
            </a-col>
            <a-col :md="6" :sm="24">
              <a-form-item label="部门编码">
                <a-input v-model="queryParam.deptCode" placeholder="请输入部门编码" allow-clear @keyup.enter.native="handleQuery"/>
              </a-form-item>
            </a-col>
            <a-col>
    ...
    <a-card :bordered="false" class="table-card">
      ...
      <advance-table
        ...
        <span slot="deptName" slot-scope="{text, record}">
          ...
          <span v-if="text.indexOf(queryParam.deptName) > -1">
        ...
        <span slot="deptCode" slot-scope="{text}">
          <span v-if="text.indexOf(queryParam.deptCode) > -1">
...
export default {
  ...
  methods: {
    ...
    /** 搜索按钮操作 */
    handleQuery () {
      if (this.queryParam.deptName === '' && this.queryParam.deptCode === '') {
         this.expandedRowKeys = []
         this.getList()
      } else {
          this.loading = true
          ...
          if (response.data.length !== 0) {
            this.getAllDeptNode(response.data)
            // this.list = this.handleTree(response.data, 'id')
            this.list = response.data
          } else {
             this.list = []
