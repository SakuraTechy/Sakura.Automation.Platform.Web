...
const CERTIFICATE_TOKEN_KEY = 'certificate1_token'
...
export default {
  ...
  methods: {
    ...
    async getAllProjectList(skipRefresh = false) {
      try {
        const buildUrl = (endpoint) => `${this.$config.environment.url}${endpoint}`
        const response = await axios.get(buildUrl(this.$config.environment.products), {
        ...
        if (!skipRefresh && isCertificateTokenExpired(error)) {
          ...
          await this.Login()
          return this.getAllProjectList(true)
        }
        throw error
      }
    },
    ...
    async ensureAuthenticated() {
      ...
      this._authPromise = (async () => {
        ...
        if (cachedToken) {
          ...
          try {
            await Promise.all([this.getAllProjectList(true), this.getList(true)])
            return
          } catch (error) {
