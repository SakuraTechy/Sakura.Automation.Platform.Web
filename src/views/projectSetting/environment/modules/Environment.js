import { getAllProject, addEnvironment, getEnvironmentInfo, editEnvironment } from '@/api/project'
  ...
  methods: {
    ...
    changeState(type, item) {
      if (type === 0) {
        this.serverList.map((server) => {
          server.status = server.id === item.id ? 1 : 0
        })
      } else {
    ...
    handleUpdate(row, ids) {
      ...
      getEnvironmentInfo(row.id).then((response) => {
        const { id, environmentName, sysProjectList, sysCommonConfigList, sysDataBaseConfigList, serverList } =
          response.data
        const projectList = []
